class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';

  static String authenBackgr = '$imagePath/authen_backgr_img.svg';

  static String googleLogoImage = '$imagePath/google_logo.png';

  // Recived images
  static String imgLockBlueGray300 = '$imagePath/img_lock_blue_gray_300.svg';

  static String imgCall = '$imagePath/img_call.svg';

  // Recived - Tab Container images
  static String imgNavShipRedA200 = '$imagePath/img_nav_ship_red_a200.svg';

  // Operate images
  static String imgPlay = '$imagePath/img_play.svg';

  static String imgCloseLightBlueA70001 =
      '$imagePath/img_close_light_blue_a700_01.svg';

  static String imgReply = '$imagePath/img_reply.svg';

  static String imgPlayGreenA700 = '$imagePath/img_play_green_a700.svg';

  static String imgCheckmark = '$imagePath/img_checkmark.svg';

  static String imgPlayOrangeA70001 = '$imagePath/img_play_orange_a700_01.svg';

  static String imgAirplane = '$imagePath/img_airplane.svg';

  static String imgEllipse15 = '$imagePath/img_ellipse_15.svg';

  static String imgThumbsUpPrimary = '$imagePath/img_thumbs_up_primary.svg';

  static String imgTelevisionBlueGray300 =
      '$imagePath/img_television_blue_gray_300.svg';

  static String imgArrowDown = '$imagePath/img_arrow_down.svg';

  static String imgNavOperateRedA200 =
      '$imagePath/img_nav_operate_red_a200.svg';

  // Setting images
  static String imgEditBlack900 = '$imagePath/img_edit_black_900.svg';

  static String imgLockPrimary = '$imagePath/img_lock_primary.svg';

  static String imgVector = '$imagePath/img_vector.svg';

  static String imgCheckmarkBlack900 = '$imagePath/img_checkmark_black_900.svg';

  static String imgNavSettingRedA200 =
      '$imagePath/img_nav_setting_red_a200.svg';

  // ONB - Oderbox images
  static String imgEditBlack90015x15 =
      '$imagePath/img_edit_black_900_15x15.svg';

  static String imgContrast = '$imagePath/img_contrast.svg';

  // Common images
  static String imgLock = '$imagePath/img_lock.svg';

  static String imgUser = '$imagePath/img_user.svg';

  static String imgUserPrimary = '$imagePath/img_user_primary.svg';

  static String imgImage = '$imagePath/img_image.png';

  static String imgTelevision = '$imagePath/img_television.svg';

  static String imgClose = '$imagePath/img_close.svg';

  static String imgCalendar = '$imagePath/img_calendar.svg';

  static String imgNavHome = '$imagePath/img_nav_home.svg';

  static String imgNavShip = '$imagePath/img_nav_ship.svg';

  static String imgNavOperate = '$imagePath/img_nav_operate.svg';

  static String imgNavSetting = '$imagePath/img_nav_setting.svg';

  static String imgComputer = '$imagePath/img_computer.svg';

  static String imgLinkedin = '$imagePath/img_linkedin.svg';

  static String imgGrid = '$imagePath/img_grid.svg';

  static String imgThumbsUp = '$imagePath/img_thumbs_up.svg';

  static String imgThumbsUpBlueGray300 =
      '$imagePath/img_thumbs_up_blue_gray_300.svg';

  static String imgEdit = '$imagePath/img_edit.svg';

  static String imgNavHomeBlueGray300 =
      '$imagePath/img_nav_home_blue_gray_300.svg';

  static String imgSave = '$imagePath/img_save.svg';

  static String imgVectorPrimary = '$imagePath/img_vector_primary.svg';

  static String imgArrowRight = '$imagePath/img_arrow_right.svg';

  static String imgCalendarBlack900 = '$imagePath/img_calendar_black_900.svg';

  static String imgLinkedinBlack900 = '$imagePath/img_linkedin_black_900.svg';

  static String imgImage10 = '$imagePath/img_image_10.png';

  static String imgArrowRightOnerrorcontainer =
      '$imagePath/img_arrow_right_onerrorcontainer.svg';

  static String imgBookmark = '$imagePath/img_bookmark.svg';

  static String imgImage11 = '$imagePath/img_image_11.png';

  static String imageNotFound = 'assets/images/image_not_found.png';

  static String personSittingBackgr = 'assets/images/person_sitting.png';

  static String authenBackgrPNG = 'assets/images/person_sitting.png';
}
